...
export default class helpCalendar {
    ...
    getDateFromFormat(date) {
        ...
        if (format.indexOf("/") !== -1) {
            ...
            throw new Error(
                "Your date format not valid. Please read documentation.!"
            )
        }
    ...
    getWeeksInMonth(month, year) {
        ...
        while (start <= numDays) {
            weeks.push({
                ...
                number: new Date(year, month, start).getWeekNumber(
                    this.sundayStart
                ),
                days: [],
            })
    ...
    getLeftMonthDays(month, year) {
        ...
        if (weekDaysCount !== 7) {
            ...
            for (
                let i = leftMonthLastWeek.start;
                i <= leftMonthLastWeek.end;
                i++
            ) {
                days.push(i)
            }
